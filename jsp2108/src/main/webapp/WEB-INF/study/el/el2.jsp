...
%>
		...
		mbc : <%=mbc %><br>
		<hr>
		<p>EL을 사용한 값의 전송 받을때는 $ {param.xxx }사용</p>
		아이디 : ${param.mid }<br>
		비밀번호 : ${param.pwd }<br>
		성명 : ${param.name }<br>
		mbc : ${param.mbc }<br>
		<p><a href="<%=request.getContextPath()%>/el1.st" class="btn btn-secondary">돌아가기</a></p>
		<hr>
<%
		int[] atom1 = new int[5];
		int atom2[] = new int[5];
...
<% 	pageContext.setAttribute("atom4", atom4); %>
		...
		<font color="red">사용 안됨 JSTL을 사용하여 배열값을 변수에 저장후 출력해야한다.<br>
		hobby[0] : $ {param.hobby[0] }<br>
		hobby[1] : $ {param.hobby[1] }<br></font>
		<hr>
		<h3>객체를 전달받기</h3>
<%
