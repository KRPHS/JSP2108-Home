...
<%@ taglib uri="http://java.sun.com/jstl/fmt" prefix="fnt" %>
...
<body>
  ...
  <p><br></p>
  <div class="container">
		<h2>JSTL(Java Standard Tag Libraries)</h2>
		<table class="table">
			<tr>
				<th>라이브러리명</th>
				<th>주소(URL)</th>
...
%>
		...
		VO객체의 name을 jstl변수 name에 담아서 출력? <c:set var="name" value="${vo.name }"/><br>
		name = ${name }<br><br>
		...
		저장된 vo객체의 name필드의 값은 : ${vo.name } 또는 <%=vo.getName() %><br>
  </div>
  <br>
