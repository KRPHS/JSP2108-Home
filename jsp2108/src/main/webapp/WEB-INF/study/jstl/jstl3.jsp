...
%>
		...
		<ul>
			<li>substringBefore() : ${fn:substringBefore(atom,'o') }</li>
			<li>substringAfter() : ${fn:substringAfter(atom,'o') }</li>
			<li>2번재로 만나는 'o'문자 뒤의 값을 모두 출력? ${fn:substringAfter(fn:substringAfter(atom, 'o'), 'o') }</li>
			<li>1번째 'o'에서 2번째 'o'사이의 문자를 출력? ${fn:substring(fn:substring(atom, fn:indexOf(atom,'o')+1, fn:length(atom)),0,fn:indexOf(atom, 'o')) }</li>
			<li>${fn:substringBefore(fn:substringAfter((atom), 'o'),'o') }</li>
		</ul> <br>
		...
		11. 특정 문자로 끝나는지의 유무를 판단? endWith(문자열, 시작문자열)
		<ul>
			<li>atom변수의 시작글자가 '!' 인지를 판단? ${fn:endsWith(atom, '!') }</li>
			<li>atom변수의 시작글자가 '?' 인지를 판단? ${fn:endsWith(atom, '?') }</li>
		</ul>
...
%>
		...
		<ul>
			...
			<li>1. 소수이하 두자리 출력(반올림) : <fmt:formatNumber value="${su2}" pattern="0.00"/> </li>
			<li>2. 정수부 출력(반올림) : <fmt:formatNumber value="${su2}" pattern="0"/> </li>
			<li>3. 정수부 출력(반올림) : <fmt:formatNumber value="${su2}" pattern="#,###"/> </li>
		...
		<ul>
			...
			<li><fmt:formatDate value="${now }" pattern="yyyy년MM월dd일"/> </li>
			<li><fmt:formatDate value="${now }" pattern="yyyy-MM-dd hh:mm:ss"/> </li>
			<li><fmt:formatDate value="${now }" pattern="yyyy년MM월dd일 hh시mm분ss초"/> </li>
		</ul><br>
		<hr>
		<h4>타임존 설정</h4>
