...
<body>
	...
	<div class="container">
		...
		<hr>
		<div>
			<input type="button" value="텍스트박스 추가" class="form-control" onclick="textBoxAppend()">
...
<script>
	...
	function textBoxAppend() {
		cnt++;
		var textBox = "";
		textBox += "<div class='' id='tBox"+cnt+"'>";
		textBox += '<input type="text" name="product'+cnt+'" id="product'+cnt+'" class="form-control" style="float:left; width:85%;">';
		textBox += '<input type="button" value="삭제" onclick="deleteBox('+cnt+')" class="btn btn-outline-secondary form-control ml-2" style="width:10%;"><br>';
		textBox += "</div>";
	...
	function textView() {
		...
		for(var i = 1; i <= cnt; i++) {
			...
			if(document.getElementById(product).innerHTML != null) {
